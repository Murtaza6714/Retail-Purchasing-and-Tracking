  ...
  $a=0;
    ...
    if (!$con){
      die("connection to  this database failed due to".mysqli_connect_error());
        ...
        $sql = "select ProductCategory,ProductName,BrandName from addproduct where ProductCategory = 'Applications'";
    ...
    while($rows = mysqli_fetch_assoc($result)){
        ...
        <input type="text" name="ProductCategory" value="<?=$rows["ProductCategory"]?>" hidden>
              ...
              <input type="text" name="BrandName" value="<?=$rows["BrandName"]?>" hidden>
               <input type="text" placeholder="Quantity" required><input type="submit" style="margin-left: 10px;" class="btn btn-primary" name="submit"></form> </td></tr><?php
        }
    }
    ?>
    </tbody>
