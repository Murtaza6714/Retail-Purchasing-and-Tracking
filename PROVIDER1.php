...
<body>
  ...
  if(isset($_POST['button'])){
    ...
    $username_search = "select * from `PROVIDER1` where Username = '$username'";

    $query = mysqli_query($con,$username_search);
  
    $password_search = "select * from `PROVIDER1` where Password = '$password'";
    ...
    if ($result->num_rows > 0){
     ...
     ?>
    <script>
    location.replace("CostumerRequest.php");
    </script>
    <?php
    }else{
      ?>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item active">
                    <a class="nav-link passive" href="#">PROVIDER 1<span class="sr-only">(current)</span></a>
                </li>
                <li class="nav-item active">
    ...
    <section class="text-gray-700 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
            <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
                <h1 class="title-font font-medium text-3xl text-gray-900">Items Provider </h1>
                <p class="leading-relaxed mt-4">Provider Fulfill the needs of costumer.</p>
            </div>
            <form class="lg:w-2/6 md:w-1/2 bg-gray-200 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0"
                method="POST">
                <h2 class="text-gray-900 text-lg font-medium title-font mb-5">PROVIDER 1_LOGIN</h2>
                <input
                ...
                <p class="text-xs text-gray-500 mt-3">Only Providers login.</p>
            </form>
        </div>
    </section>
