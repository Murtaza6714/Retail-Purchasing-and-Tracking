...
<?php
  if(isset($_POST['register'])){
    ...
    $emailquery = "select * from `registrationpage` where Email = '$email'";
    ...
    $sql = "INSERT INTO `registrationpage` (`FullName`, `Email`, `Username`, `Password`, `ConfirmPassword`) VALUES ('$name', '$email', '$username', '$pass', '$cpass')";
    $query = mysqli_query($con,$sql);
    if($query){
        ?>
        <script> alert("registration successfull");</script>
        <script> location.replace("COSTUMERpage.php");</script>
        <?php
    }else{
        ?>
...
if(isset($_POST['button'])){
  $con= mysqli_connect('localhost','root','','retail purchasing and tracking');
    ...
    $username_search = "select * from `registrationpage` where Username = '$username1'";

    $result = $con->query($username_search);
    ...
    if($result->num_rows >0){
      ...
      ?>
      <script> location.replace("ViewCategories.php");</script>
      <?php 
        }else{
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item active">
              ...
              <li class="nav-item active">
                <a class="nav-link passive" href="#">COSTUMER <span class="sr-only">(current)</span></a>
              </li>
              <li class="nav-item active">
      ...
      <section class="text-gray-700 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
          <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
            <h1 class="title-font font-medium text-3xl text-gray-900">Costumer Page</h1>
            <p class="leading-relaxed mt-4">New costumer register here or login</p>
          </div>
          <form class="lg:w-2/6 md:w-1/2 bg-gray-200 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0" method="POST" >
            <h2 class="text-gray-900 text-lg font-medium title-font mb-5">COSTUMER_LOGIN</h2>
            ...
            <input type="Submit" placeholder=" SSubmit" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg" name="button">
            <br><button type="button" class="btn btn-danger">reset</button>
            <p class="text-xs text-gray-500 mt-3">Only COSTUMERS login.</p>
            <p class="text-xs text-gray-500 mt-3">New user?</p>
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form method="POST" action="COSTUMERpage.php" >
                  <div class="modal-body">
                  <input type="text" class="form-control" id="formGroupExampleInput" placeholder="Full name" name="name"><br>
                  <input type="email" class="form-control" id="formGroupExampleInput" placeholder="Email" name="email"><br>
                  <input type="text" class="form-control" id="formGroupExampleInput" placeholder="Username" name="username"><br>
                  <input type="password" class="form-control" id="formGroupExampleInput" placeholder="Password" name="password"><br>
                  <input type="password" class="form-control" id="formGroupExampleInput" placeholder="Confirm Password" name="cpassword"><br>
  </div>
